...
%
% Create Lombard-GRID-2mix dataset without background noise (2-speaker mixtures; min version; 8/16 kHz fs).
% This script assumes that the Audio-Visual Lombard GRID Speech Corpus (split into train, validation and test) and
    ...
    for i_type = 1:length(data_type)
        ...
        for i = 1:num_files
            ...
            if useaudioread
                ...
                [s1, fs] = wavread([lombardgrid_root C{1}{i}]); %#ok<*DWVRD>
                s2       = wavread([lombardgrid_root C{3}{i}]);            
            end
